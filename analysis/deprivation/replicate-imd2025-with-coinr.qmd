...
## Estimate the factor-analysis weights
...
1. convert each indicator to the rank-based normal distribution defined above;
2. fit a one-factor maximum-likelihood model with `stats::factanal()` and no rotation;
3. orient the factor so that positive loadings mean more deprivation; and
4. divide each loading by the sum of the loadings.
...
crime_factor <- derive_factor_weights(crime_factor_data)
```

For verification only, compare the estimated weights with the rounded weights printed in the Technical Report. The reconstruction below uses the estimated values, not the published values.

```{r}
#| label: factor-weight-comparison
...
knitr::kable(factor_weight_comparison, digits = 4)
```
